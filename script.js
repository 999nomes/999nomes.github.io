...
console.log = (...args) => {
    originalLog(...args);
    appendDebug(args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' '), 'log');
};
console.error = (...args) => {
    originalError(...args);
    appendDebug(args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' '), 'error');
};
console.warn = (...args) => {
    originalWarn(...args);
    appendDebug(args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' '), 'warn');
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('App inicializado. Debugger pronto.');
    // === DOM Elements ===
